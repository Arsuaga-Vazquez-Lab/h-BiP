...
def trigram_3slides(df):
    ...
    def trigram_3slides_oneseq(seq):
        ...
        for i in range(n):
            word_lists[i % 3].append(seq[i:i+3])
        return word_lists
    trigram = []
    accession = []
    slide = []
    ...
    for i in range(df.shape[0]):
        ...
        human_triplicate = [df.iloc[i].Human]*3
        trigram.extend(trigram_seq)
        accession.extend(acc_triplicate)
        human.extend(human_triplicate)
        slide.extend(['slide1', 'slide2', 'slide3'])
    df_target = pd.DataFrame({'Accession': accession, 'Slide': slide, 'Human': human})
...
def create_seq_embedding(_trigram, _embedding_matrix, _word2id, _unknown_vector, _word_vector_size=100):
    ...
    Parameters
    ----------
    _trigram : numpy.ndarray
        Numpy array (samples, trigram from sequence)
    _embedding_matrix : numpy.ndarray
...
def embedding(train, test):
    ...
    """
    sars1_3w_train, train_target = trigram_3slides(train)
    train_target['Source'] = ['Train'] * (train_target.shape[0])
    sars1_3w_test, test_target = trigram_3slides(test)
